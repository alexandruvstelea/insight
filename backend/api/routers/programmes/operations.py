...
class ProgrammeOperations:
    ...
    async def get_programmes(self) -> List[ProgrammeOut]:
        try:
            query = select(Programme).options(joinedload(Programme.faculty))
            result = await self.session.execute(query)
            programmes = result.scalars().unique().all()
            if programmes:
                return sorted(list(programmes), key=lambda x: x.name)
    ...
    async def add_programme(self, programme_data: ProgrammeIn) -> ProgrammeOut:
        try:
            ...
            new_programme.faculty = await id_to_faculty(
                self.session, programme_data.faculty_id
            )
    ...
    async def update_programme(
        ...
        try:
            ...
            if programme:
                ...
                if new_programme_data.faculty_id:
